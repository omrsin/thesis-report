...
\begin{tikzpicture}[auto]
	...
	\node [block, minimum height=2cm, minimum width=2cm] at (JPF.west) (APP) {\txt{\textbf{Application} \\ \textit{(bytecode)}}};
...
%\node [longBlock] (JPF) {\textbf{JPF Core}};	
...
%\node [block-double, left of=IFRETURN, node distance=3cm] (RETURN) {\textbf{return} -1{;}};
	...
	\path [arrow] (JPFCORE) -- (FACTORY);
	\path [arrow] (JPFCORE) -- (CHOICE);
	\path [arrow] (JPFCORE) -- (LISTENER);
	\path [arrow] (JPFCORE) -- (NATIVE);
	\path [arrow] (JPFCORE) -- (PUBLISHER);
	\path [arrow] (JPFCORE) -- (SEARCH);
	
	\path [arrow] (FACTORY)   -- (EXTENSIONS);
	\path [arrow] (CHOICE) 	  -- (EXTENSIONS);
	\path [arrow] (LISTENER)  -- (EXTENSIONS);
	\path [arrow] (NATIVE) 	  -- (EXTENSIONS);
	\path [arrow] (PUBLISHER) -- (EXTENSIONS);
	\path [arrow] (SEARCH) 	  -- (EXTENSIONS);
