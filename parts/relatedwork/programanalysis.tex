...
%\begin{figure}[t]
%\[\xymatrix@C=0.3em{
%	. & . & \txt{public static int search(int[] a, int elem)} & . & .
...
\tikzstyle{block-double} = [rectangle, double distance =1pt, draw, text centered, rounded corners, minimum height=2em]
...
\begin{tikzpicture}[node distance = 2cm, auto]
	...
	\node [block-double, below of=FORINIT] (RETURN) {\textbf{return} -1{;}};
	% Draw edges
	\draw [line, rounded corners] (INIT) -- (-5,0) |- (FORINIT.west);
