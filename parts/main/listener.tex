...
\tikzstyle{line-dashed} = [draw, dashed, -{>[length=2mm, width=1mm]}]
\begin{figure}[t]
	\centering
...
%. filter (v2 -> v2 > 2);
	...
	\node [invisible-block, below of=C1, minimum width=0.2cm](C2){};
	\node [invisible-block, below of=C2, minimum width=1cm](C3){};
	\node [invisible-block, below of=C3](C4){};
	\node [invisible-block, below of=C4](C5){};
	\node [invisible-block, below of=C5, minimum width=3.5cm](C6){};
	\node [invisible-block, below of=C6, minimum width=4.5cm](C7){};
	\node [invisible-block, below of=C7, xshift=0.6cm](C8){};
	\node [invisible-block, below of=C8, xshift=-0.6cm, minimum width=3.5cm](C9){};
	
	\node [block, right of=CODE,  drop shadow, node distance=6cm, minimum height=5cm, minimum width=3cm] (LISTENER) {Spark \\ Method \\ Listener};
...
%	\node [block, right of=BACKTRACK, double distance=1pt] (TERMINATE) {\small Terminate \\ \small Execution};			
	% Draw edges
	\path [line-dashed] (C2.east) -- (L2.west);
	\path [line-dashed] (C3.west) -- (L3.west);
	\path [line-dashed] (C7.west) -- (L7.west);
	\path [line-dashed] (C8.east) -- (L8.west);
	\path [line-dashed] (C9.west) -- (L9.west);
