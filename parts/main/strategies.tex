...
\begin{figure}[h]
	...
	% Draw edges
	\path [line] (INIT) -- node[]{\small $SYM\_EXPR$}(FILTER.apex);
	\path [line] (FILTER.right corner) -- node[below]{\textit{true}} node[]{\small $SYM\_EXPR$}(TRUE);
	\path [line] (FILTER.left corner) -- node[below]{false}(BACKTRACK);
	
	\end{tikzpicture}
	\caption[Diagram of the \textit{filter} Strategy]{Diagram of the \textit{filter} strategy. The input parameter is initialized to the symbolic expression percolated by the previous Spark operation if there is any, otherwise is a new symbolic expression. Filter transformations always produce at least two branches, one that satisfy the predicate and one that does not. In the case of the branch that does not satisfy it, the execution is terminated and an immediate backtrack is triggered. The output of the transformation is always the same input symbolic expression disregarding the path taken.}
...
\begin{figure}[h]
	...
	% Draw edges
	\path [line] (INIT) -- node[]{\small $SYM\_EXPR$}(MAP.apex);
	\path [line] (MAP.right corner) -- node[]{\small $SYM\_EXPR^I$}(SCN1);
	\path [line] (MAP.30) -- node[below=3mm]{...} node[]{\small $SYM\_EXPR^{II}$}(SCN2);	
	\path [line] (MAP.left corner) -- node[]{\small $SYM\_EXPR^N$}(SCN3);
	
	\end{tikzpicture}
	\caption[Diagram of the \textit{map} Strategy]{Diagram of the \textit{map} strategy. The input parameter is initialized to the symbolic expression percolated by the previous Spark operation if there is any, otherwise is a new symbolic expression. The output value percolated to the following functions is a new symbolic expression derived from any operations applied on the symbolic input. Map transformations do not necessarily have a branching condition, however, in the case there are, it is most likely that the output value will be different.}
...
\begin{figure}[t]
	...
	% Draw edges
	\path [line] (INIT) -- node[]{\small $(CONC, SYM\_EXPR)$}(REDUCE.west);
	...
	% Draw edges
	\path [line] (INIT) -- node[]{\small $(SYM\_EXPR^I, SYM\_EXPR^{II})$}(REDUCE.west);
	\path [line] (REDUCE.east) -- (BACKTRACK);	
	\end{tikzpicture}

	\begin{minipage}[t]{\linewidth}
		\subcaption{Reduce strategy with the accumulated value taken as a symbolic input.}
		\label{fig:strategies:reduce:symbolic}
	\end{minipage}
	
	\caption[Diagrams of the \textit{reduce} Strategy]{Diagrams of the \textit{reduce} Strategy. The two modes of execution are shown here. The first considers the accumulated input parameter as concrete, this translates to only considering branching operations applied on single elements of the \acrshort{acr:rdd}. The other mode considers the accumulated to be a symbolic input as well. A backtrack is always triggered after a reduce action.}
\end{figure}

\textit{Post-processing}

Independently of the output of the \textit{reduce} action, the post-processing phase of the reduce strategy always triggers a termination of the current execution thread and a backtrack. This is because of the nature of Spark actions which indicate the culmination of the processing of an \acrshort{acr:rdd}. However, during the analysis of the reduce action, there might be branching operations executed on the accumulated or the single parameter, which will cause \spf to register a choice generator and explore all the possible paths. Ultimately, all these paths will lead to a backtrack after the end of the \textit{reduce} action.

%TODO: The second mode is not yet complete
...
\tikzstyle{invisible-block} = [rectangle, draw=none, fill=none, align=center, minimum height=3em, minimum width=3em]
\tikzstyle{triangle} = [isosceles triangle, draw, fill=white, align=center, shape border rotate=-180, minimum height=3em, minimum width=4cm, drop shadow]
\tikzstyle{line} = [draw, -{>[length=2mm, width=1mm]}]
\begin{figure}[h]
	\centering
	...
	\node [block, right of=FLATMAP] at (FLATMAP.5) (CG2) {$CG^{II}$};
	\node [block, right of=FLATMAP] at (FLATMAP.left corner) (CG3) {$CG^{N}$}; 
	\node [invisible-block, right of=CG1, node distance=4cm] at (CG1.north east) (SCN11) {...};
	\node [invisible-block, right of=CG1, node distance=4cm] at (CG1.5) (SCN12) {...};
	...
	\node [invisible-block, right of=CG2, node distance=4cm] at (CG2.5) (SCN22) {...};
	\node [invisible-block, right of=CG2, node distance=4cm] at (CG2.south east) (SCN23) {...};
	\node [invisible-block, right of=CG3, node distance=4cm] at (CG3.north east) (SCN31) {...};
	\node [invisible-block, right of=CG3, node distance=4cm] at (CG3.5) (SCN32) {...};
	\node [invisible-block, right of=CG3, node distance=4cm] at (CG3.south east) (SCN33) {...};
%	\node [invisible-block, right of=CG2] (SCN2) {...};
%	\node [invisible-block, right of=CG3] (SCN3) {...};
	
	% Draw edges
	\path [line] (INIT) -- node[]{\footnotesize $SYM\_EXPR$}(FLATMAP.apex);
	\path [line] (FLATMAP.right corner) -- (CG1);
%	\path [line] (FLATMAP.right corner) -- node[]{\footnotesize $SYM\_EXPR^I$}(SCN1);
	\path [line] (FLATMAP.5) -- node[below=7mm]{...} (CG2);	
	\path [line] (FLATMAP.left corner) -- (CG3);
	\path [line] (CG1.north east) -- node[]{\footnotesize $SYM\_EXPR^{I\_I}$}(SCN11);
	\path [line] (CG1.5) -- node[]{\footnotesize $SYM\_EXPR^{I\_II}$}(SCN12);
	\path [line] (CG1.south east) -- node[]{\footnotesize $SYM\_EXPR^{I\_M}$}(SCN13);
	
	\end{tikzpicture}
	\caption[Diagram of the \textit{flatMap} Strategy]{Diagram of the \textit{flatMap} strategy. The input parameter is initialized to the symbolic expression percolated by the previous Spark operation if there is any, otherwise is a new symbolic expression. The output value percolated to the following functions is taken from a special choice generator registered when the \textit{flatMap} transformation is done. The choice generator contains all the different elements contained in the collection. Every path taken during the \textit{flatMap} transformation registers a new choice generator with all possible values returned in that particular path.}
